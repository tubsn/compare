...
<div style="float:right; position:relative; top:-13px;" class="text-right">
	<a class="button light" href="/readers/map/local">Lokale Karte</a>
	-
	<a class="button" href="/readers/map/germany">Deutschlandkarte</a>
</div>
...
function interactiveSVG(container) {
	...
	svg.addEventListener('mouseover', (e) => {
		...
		if (orders) {
			...
			tooltip.innerHTML = `<b>PLZ:</b> ${plz}xx<br/><b>Users:</b> ${orders}`;
...
function interactiveSVG2() {
	...
	Array.from(svgpaths).forEach(path => {
	    ...
	    path.addEventListener('mouseover', (e) => {
			...
			if (count) {
				counter.innerHTML = `<b>PLZ:</b> ${plz}xx <b>Users:</b> ${count}`;
