...
width:18px; height:18px; margin-left:0.3em; top:3px; background-image:url('/styles/img/flundr/icon-delete-black.svg'); opacity:0.2; background-size:cover;}
...
<main class="auth-layout">
	...
	<?php foreach ($users as $user): ?>
		<tr>
			<td><?=$user['id']?></td>
			<td><a href="/admin/users/<?=$user['id']?>"><?=$user['email']?></td>
			...
			<td>
				...
				<fl-dialog selector="#dialog-delete-<?=$user['id']?>" href="/admin/users/<?=$user['id']?>/delete/<?=$CSRFToken?>">
				<h1>Bestätigen</h1>
	...
	<a class="button mt" href="/admin/users/new">Neuen User anlegen</a>
